...
void fadeLED(uint32_t adcResult)
{
    uint32_t adcResult_1 = (adcResult * 50000) / 4095;
...
//    g_fadeValue += g_fadeDir * FADE_STEP;                           /* Calculation of the new duty cycle            */
    setDutyCycle(adcResult_1);                                      /* Set the duty cycle of the PWM                */
}
...
void makeSound (unsigned int soundIdx) {
    ...
    uPeriod = (unsigned short)(10000000/fBuzz[soundIdx]);
    g_tomConfig.period = uPeriod;
    g_tomConfig.dutyCycle = (unsigned short)(uPeriod*0.1f);
